...
class SARSAPlayer(Player):
    ...
    def updateQ(self, last_state: np.ndarray, cur_state: np.ndarray, last_step: int, reward: float):
        ...
        next_step = self.get_step(cur_state)

        self.Q[last_state][last_step] = np.clip(
            (1-self.alpha)*self.Q[last_state][last_step] + self.alpha*(reward + self.gamma*self.Q[cur_state][next_step]), -100, 100)
...
if __name__ == "__main__":
    ...
    ROW = 5
    COL = 4
    PATH = f"saved_model/tabular_sarsa_{ROW}_{COL}.pkl"
    ...
    p1 = SARSAPlayer(Q, ROW, COL)

    if MODE == 0:
        p2 = SARSAPlayer(Q, ROW, COL)
        # start training
